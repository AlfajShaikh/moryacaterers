...
export function Orders() {
    ...
    const calculatePerPlatePrice = (order) => {
        ...
        order.shifts?.forEach((shift) => {
            shift.selectedItems?.forEach((item) => {
    ...
    const handleItemPriceChange = (orderId, shiftIndex, itemIndex, newPrice) => {
        setLocalOrders((prevOrders) =>
            prevOrders.map((order) => {
                if (order._id !== orderId) return order;
                const updatedOrder = { ...order };
                updatedOrder.shifts[shiftIndex].selectedItems[itemIndex].price = newPrice;
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 py-8 md:py-10">
                ...
                {(!filteredAndSortedOrders || filteredAndSortedOrders.length === 0) && (
                    <div className="bg-white/80 backdrop-blur-xl border border-white rounded-3xl p-12 text-center shadow-sm">
                        <ShoppingBagIcon className="w-20 h-20 text-slate-300 mx-auto mb-4" />
                        <h2 className="text-2xl font-bold text-slate-700">कोणतीही ऑर्डर सापडली नाही</h2>
                        <p className="text-slate-500 mt-2">तुमचे फिल्टर्स बदलून पुन्हा प्रयत्न करा.</p>
                    </div>
                )}
                            ...
                            >
                                ...
                                {isExpanded && (
                                    <div className="border-t border-slate-200/60 p-5 md:p-6 lg:p-8 bg-slate-50/50 rounded-b-3xl animate-in slide-in-from-top-4 fade-in duration-300">
                                        {/* Additional Editable Info Section */}
                                        <div className="bg-white border border-slate-200 p-5 rounded-2xl mb-6 shadow-sm">
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                                {/* Guest Count */}
                                                <div>
                                                    <label className="flex items-center gap-1.5 text-sm font-bold text-slate-600 mb-1.5"><UsersIcon className="w-4 h-4" /> Guest Count (पाहुण्यांची संख्या)</label>
                                                    <input
                                                        type="number"
                                                        placeholder="उदा. 500"
                                                ...
                                                <div>
                                                    ...
                                                    <textarea
                                                        ...
                                                        onChange={(e) => handleFieldChange(order._id, "specialInstructions", e.target.value)}
                                                ...
                                                {/* Special Services */}
                                                <div>
                                                    <label className="flex items-center gap-1.5 text-sm font-bold text-slate-600 mb-1.5"><SparklesIcon className="w-4 h-4" /> Special Services (Add-ons)</label>
                                                    <textarea
                                                        rows="2"
                                                        placeholder="उदा. VIP सर्व्हिस, डेकोरेशन..."
                                                        value={order.specialServices || ""}
                                                        onChange={(e) => handleFieldChange(order._id, "specialServices", e.target.value)}
                                                        className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-blue-400 outline-none transition resize-none"
                                                    ></textarea>
                                                </div>
                                            </div>
                                        ...
                                        {/* Shifts and Items (Editable Prices) */}
                                            ...
                                            {order.shifts.map((shift, shiftIdx) => (
                                                <div key={shiftIdx} className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                                                    ...
                                                    <div className="bg-gradient-to-r from-slate-50 to-white px-5 py-3 border-b border-slate-100 flex items-center gap-2">
                                                        <ClockIcon className={`w-5 h-5 ${shift.shift === "Morning" ? "text-amber-500" : shift.shift === "Evening" ? "text-orange-500" : "text-indigo-500"}`} />
                                                    ...
                                                    {/* Shift Items List */}
                                                    <div className="p-2">
                                                        {shift.selectedItems.length > 0 ? (
                                                            <div className="space-y-1">
                                                                {shift.selectedItems.map((item, itemIdx) => (
                                                                    <div key={itemIdx} className="flex justify-between items-center p-2.5 rounded-xl hover:bg-slate-50 transition-colors">
                                                                        ...
                                                                        <div className="flex items-center bg-white border border-slate-200 rounded-lg focus-within:ring-2 focus-within:ring-blue-400 overflow-hidden">
                                                                            <span className="bg-slate-50 px-2 py-1 text-slate-500 font-bold border-r border-slate-200">₹</span>
                                                                            <input
                                                                                type="number"
                                                                                value={item.price}
                                                                                onChange={(e) => handleItemPriceChange(order._id, shiftIdx, itemIdx, e.target.value)}
                                                                                className="w-20 px-2 py-1 outline-none text-right font-bold text-slate-800"
                                                                            />
                                                                        </div>
                                                                    </div>
                                                                ))}
