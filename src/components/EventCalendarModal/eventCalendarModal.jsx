            ...
            <div className="bg-white rounded-[2rem] w-full max-w-4xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 relative flex flex-col max-h-[95vh]">
                ...
                <div className="bg-slate-50 border-b border-slate-100 p-5 flex items-center justify-between flex-shrink-0 z-20 relative">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-xl font-black text-slate-800">कार्यक्रमाचे वेळापत्रक</h2>
                            <p className="text-sm font-semibold text-slate-500 tracking-wide">Event Schedule</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="relative flex-1 overflow-y-auto p-5 md:p-8 bg-white z-0">
                    ...
                    <div className="grid grid-cols-7 gap-1.5 md:gap-3 z-10 relative">
                        ...
                        {Array.from({ length: firstDayOfMonth }).map((_, idx) => (
                            <div key={`empty-${idx}`} className="h-20 md:h-28 rounded-2xl bg-slate-50/50 border border-transparent"></div>
                        ))}
                        ...
                        {Array.from({ length: daysInMonth }).map((_, idx) => {
                            ...
                            return (
                                <div
                                    ...
                                    className={`relative flex flex-col h-20 md:h-28 p-1.5 md:p-2 rounded-2xl border transition-all duration-300 overflow-hidden
                                    ...
                                    `}
                                >
                                    <span className={`text-xs md:text-sm font-black mb-1 ${dateTextClass}`}>
                                        {date}
                                    </span>
                                        ...
                                        {dayEvents.map((event, eventIdx) => {
                                            ...
                                            return (
                                                <div
                                                    ...
                                                    // THIS IS THE FIX: Individual click handler that stops propagation
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                                                        setSelectedEvent(event);
                                                    }}
                                                    // Added a subtle hover state and cursor-pointer for better UX
                                                    className={`flex items-center gap-1.5 text-[9px] md:text-[11px] font-bold truncate p-0.5 rounded cursor-pointer hover:bg-slate-900/5 transition-colors ${statusStyle.text}`}
                    ...
                    {/* Legend */}
                    <div className="mt-6 pt-4 border-t border-slate-100 flex flex-wrap items-center justify-end gap-4 text-[10px] md:text-xs font-bold text-slate-500">
